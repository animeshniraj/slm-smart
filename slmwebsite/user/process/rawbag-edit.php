	...
	if($editidPermission&& $creationPermission)
			{
				?>
				<div class="col-sm-12">
				<button type="button" class="btn btn-primary m-b-0 pull-left" onclick="window.open('/user/print/rawbag-tag.php?processid=<?php echo $processid; ?>&grade=<?php echo $currGradeName; ?>&quantity=<?php echo getTotalQuantity($processid) ?>')"><i class="icofont icofont-barcode"></i>Generate Label</button>
...
<?php
	...
	if($result->num_rows>0)
	{
		...
		?>
<h5 style="text-align:center; background-color:#546679;padding:10px;color:#fff;">All Raw Bag Test Results</h5>
...
<div class="form-group row">
		...
		<div class="col-sm-3">
				...
				<input type="text" id="add-mid" class="form-control form-control-uppercase pull-right" placeholder="Melting ID">
		</div>
...
<script type="text/javascript">
	...
	function searchfurnace()
	{
	 	...
	 	document.getElementById("searchfurance-modal-save").disabled = true;
	 		...
	 		postData.append("action","getAllRemainingMelting");
	...
	function addparentbyid()
	{
       	...
       	for(var i=0;i<tbodyobj.children.length;i++)
       	{
       		if(tbodyobj.children[i].children[0].children[1].value==processid)
       		{
       			...
       			Swal.fire({
									icon: "error",
									title: "Error",
									html: "Melting ID already linked.",
        ...
        postData.append("action","getMeltingQuantity");
