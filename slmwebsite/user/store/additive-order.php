	...
	<tbody>
		...
		<tr>
			...
			<script type="text/javascript"> 
				...
				additivedata['<?php echo $additive[0]; ?>'] = [this.document.getElementById('additive-req-<?php echo $k; ?>'),this.document.getElementById('additive-proc-<?php echo $k; ?>')]</script>
		</tr>
...
<script type="text/javascript">
	...
	function addtolist()
	{
		...
		if(!quantity || gradename=="Choose a grade")
		{
			return;
		}
				...
				tr.innerHTML = "<td>"+count+"</td><td>"+gradename+"</td><td>"+quantity+"</td><td><button type=\"button\" class=\"btn btn-danger\" onclick=\"this.closest('tr').remove();reeval();\"><i class=\"fa fa-trash\"></i>Remove</button></td>"
	...
	function reeval()
	{
		...
		Object.keys(additivedata).forEach(function (key) {
			...
			additivedata[key][1].innerHTML=0;
		});
		...
		for(var i=0;i<tbody.children.length;i++)
		{
			...
			var qty = parseFloat(curr.children[2].innerHTML);
			...
			for(var j=0;j<c_grade_data.length;j++)
			{
				...
				if(req!=0)
				{
					...
					var proc = req
				        ...
				        postData.append("action","getfifobatch");
				        postData.append("additive",currAdditive);
				        postData.append("quantity",proc);
				        ...
				        xmlhttp.onreadystatechange = function() {
				          ...
				          // console.log(this.responseText)
				            ...
				            if(data.response)
				            {
				            	console.log(currAdditive,data.result)
				                additivedata[data.additive][1].innerHTML = data.result;
				            }
				            else
				            {
