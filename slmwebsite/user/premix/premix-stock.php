<?php
    ...
    $PAGE = [
        "Page Title" => "SLM | Premix Stock Report",
        "Home Link"  => "/user/",
        "Menu"		 => "premix-stock",
	...
	if($currgrade=="all")
    {
    	$result = runQuery("SELECT * FROM premix_batch WHERE   entrydate>= '$starttime' AND entrydate <= '$stoptime' ORDER BY entrydate");
    }
    else
    {
    	$dumgradepattern = $currgrade."#%";
    	$result = runQuery("SELECT * FROM premix_batch WHERE  entrydate>= '$starttime' AND entrydate <= '$stoptime' AND (gradename='$currgrade' OR gradename LIKE '$dumgradepattern') ORDER BY entrydate");
    ...
    while($row=$result->fetch_assoc())
    {
    	...
    	$dum["id"] = $row["premixid"];
    	$dum["mass"] = $row["mass"];
    	$dum["remaining"] = $row["mass"];

    	$dum["entrydate"] = $row["entrydate"];
    	$dum["test"] = [];
    	$dum["child"] = [];
    	$dum["grade"] = $row["gradename"];
    	$currid = $row["premixid"];
    	...
    	$result2 = runQuery("SELECT * FROM dispatch_invoices WHERE batch='$currid'  AND cid in (SELECT cid from dispatch WHERE (entrydate BETWEEN '$start' AND '$stoptime') )");
    	...
    	$result2 = runQuery("SELECT DISTINCT(param) FROM premix_batch_testparams WHERE premixid = '$currid'");
    	...
    	$result2 = runQuery("SELECT * FROM premix_batch_testparams WHERE premixid='$currid'");
    	while($row2=$result2->fetch_assoc())
    	{
	...
	<div class="row align-items-end">
		<div class="col-lg-8">
			<div class="page-header-title">
				...
				<div class="d-inline">
					<h3>Premix Stock</h3>
					<span>View Premix stock details</span>
				</div>
			</div>
		</div>
	</div>
...
<h5>Select Date Range to see Premix report</h5>
<hr>
<div class="row">
...
<tr  style="font-size:14px;">
	...
	<td width="5%"><a target="_blank" href="/user/report/basic-premix.php?id=<?php echo $data["id"]; ?>"><?php echo $data["id"]; ?></a></td></td>
