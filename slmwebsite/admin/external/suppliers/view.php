...
<?php
	...
	if($result->num_rows>0)
	{
		while($row=$result->fetch_assoc())
		{
			...
			$result2 = runQuery("SELECT * FROM external_param WHERE externalid='$currid' AND ordering=-1 AND param ='Additives'");
			$alladditives = "";
			while($row = $result2->fetch_assoc())
			{
				
					$alladditives = $alladditives .$row["value"] ."<br>";
