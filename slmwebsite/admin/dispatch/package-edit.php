<?php
    ...
    if(isset($_POST["addcomposition"]))
    {
    	...
    	for($i=0;$i<count($list);$i++)
    	{
    		...
    		{
    			runQuery("INSERT INTO premix_grade_compositions VALUES(NULL,'$gradename','$currL','$currV','$currS','1','$tol','$step')");
    		}
    		else
    		{
    			runQuery("INSERT INTO premix_grade_compositions VALUES(NULL,'$gradename','$currL','$currV','$currS','0','$tol','$step')");
    		}
    	}
    ...
    {
    	array_push($compositions,[$row["additive"],$row["composition"],$row["splits"],$row["over"]==1?"checked":" ",$row["tol"],$row["step"]]);
    }
...
<div class="form-group row">
		...
		<table class="table">
			...
			<th rowspan="1" colspan="1" >Tolerance (%)</th>
			<th rowspan="1" colspan="1" >Step(kg)</th>
...
?>
	...
	addNewAdditive('<?php echo $currC[0]; ?>','<?php echo $currC[1]; ?>','<?php echo $currC[2]; ?>','<?php echo $currC[3]; ?>','<?php echo $currC[4]; ?>','<?php echo $currC[5]; ?>')
...
?>
	...
	function addAdditive()
	{
		...
		selectObj.remove(curr);
		addNewAdditive(curr,0,1,"")
	}



	function addNewAdditive(name,value,splits,ischecked,tol,step)
	{
		...
		dumTD = document.createElement("TD");
		dumTD.innerHTML = "<input required type='number' name='tols[]' min='1' max='100' step='0.1' value='"+tol+"'>";
		row.appendChild(dumTD);
