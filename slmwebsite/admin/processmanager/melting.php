<?php
    ...
    if(isset($_POST["updateprocess1"]))
    {
    	...
    	runQuery("DELETE FROM furnaces WHERE processname='$processname'");

    	$furnacename = $_POST['furnace-name'];
    	$furnaceprefix = $_POST['furnace-prefix'];
    	$furnacespec = $_POST['furnace-specification'];

    	

    	for($i=0;$i<count($furnacename);$i++)
    	{
    		
    		runQuery("INSERT INTO furnaces VALUES(NULL,'$furnacename[$i]','$furnaceprefix[$i]','$furnacespec[$i]','$processname')");
...
<input type="hidden" name="currtab" value="creation-tabdiv">
	...
	<div class="form-group row">
		...
		<div class="col-md-10">
		<button type="button" onclick="addpropFn('furnace')" class="btn btn-primary m-b-0"><i class="fa fa-plus"></i>Add Furnace</button>
		</div>
	</div>
...
<form method="POST">
	...
	<div id = "process2-propdiv">
		...
		<div class="form-group row">
			...
			?>
			<div class="col-md-1">
				<input required readonly type="text" class="form-control" name="process2-proporder[]" placeholder="Ordering" value="<?php echo $row["ordering"]; ?>">
				<span class="messages"></span>
			</div>
			<input type="hidden" name="process2-locked[]" value="true">
			<?php
